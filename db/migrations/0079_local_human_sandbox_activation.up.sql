-- WEB-027K1 approved local credential generation; hosted role profiles remain exact.
...
CREATE OR REPLACE FUNCTION provision_public_beta_human_wallet_identity(
...
AS $$
DECLARE
  expected_human_capabilities JSONB :=
  ...
  expected_principal_capabilities JSONB :=
  ...
  login_role RECORD;
BEGIN
  -- This local generation is unavailable on hosted databases and other hosts.
  IF (current_database() = 'ipo_one_web027_candidate' AND issuer_value IN ('https://127.0.0.1:8935','https://127.0.0.1:8936')
      OR current_database() = 'ipo_one_web027_proof' AND issuer_value IN ('https://127.0.0.1:8945','https://127.0.0.1:8946'))
     AND client_id_value ~ '^client_web027[jk]_actor_(human_borrower|principal_controller)_pilot$' THEN
    expected_human_capabilities := expected_human_capabilities || '["wallet.account_binding.prepare.owned","wallet.account_binding.submit.owned","wallet.account_binding.read.owned","wallet.account_binding.revoke.owned","wallet.capabilities.discover"]'::jsonb;
    expected_principal_capabilities := expected_principal_capabilities || '["wallet.account_binding.prepare.owned","wallet.account_binding.submit.owned","wallet.account_binding.read.owned","wallet.account_binding.revoke.owned","wallet.capabilities.discover"]'::jsonb;
  END IF;

  IF (current_database() = 'ipo_one_web027_candidate' AND issuer_value IN ('https://127.0.0.1:8935','https://127.0.0.1:8936')
      OR current_database() = 'ipo_one_web027_proof' AND issuer_value IN ('https://127.0.0.1:8945','https://127.0.0.1:8946'))
     AND client_id_value ~ '^client_web027k_actor_(human_borrower|principal_controller)_pilot$' THEN
    expected_human_capabilities := expected_human_capabilities || '["subject.activate.sandbox.self"]'::jsonb;
  END IF;
