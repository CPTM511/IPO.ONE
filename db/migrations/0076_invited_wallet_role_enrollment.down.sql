-- Preserve enrolled identities and immutable event history. Use runtime rollback
-- while this guard is present; never delete authority history to downgrade.
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM authentication_role_enrollments WHERE role_bundle IN ('capital_partner_operator', 'risk_operator'))
     OR EXISTS (SELECT 1 FROM authentication_events WHERE payload->>'roleBundle' IN ('capital_partner_operator', 'risk_operator'))
     OR EXISTS (SELECT 1 FROM authentication_wallet_transactions WHERE requested_role IN ('capital_partner_operator', 'risk_operator')) THEN
    RAISE EXCEPTION 'Invited role history exists; retain schema and use compatible runtime rollback';
  END IF;
END $$;
...
ALTER TABLE authentication_role_enrollments ADD CONSTRAINT authentication_role_enrollments_role_bundle_check CHECK (role_bundle IN ('human_borrower', 'principal_controller'));
ALTER TABLE authentication_wallet_transactions DROP CONSTRAINT authentication_wallet_transactions_requested_role_check;
ALTER TABLE authentication_wallet_transactions ADD CONSTRAINT authentication_wallet_transactions_requested_role_check CHECK (requested_role IS NULL OR requested_role IN ('human_borrower', 'principal_controller'));
