...
function fail(message) {
  throw new Error(`hypercore_002d_stable_once_error: ${message}`);
}
...
export function composeHypercore002dJitExecution({
  approvedIntent,
  approval,
  readiness,
  now
  ...
  const order = approvedIntent.hyperliquidAction.orders[0];
  ...
  if (expiresAfter <= now.getTime()) fail("JIT signing interval already expired");
...
async function runCli() {
  ...
  const intentHash = process.env.IPO_ONE_HYPERCORE_STABLE_INTENT_HASH;
  const exactMarker = HASH.test(intentHash ?? "")
    ? `HYPERLIQUID-002D:${intentHash}`
    : null;
  if (
    exactMarker === null ||
    process.env.IPO_ONE_APPROVE_HYPERCORE_TESTNET_PROOF !== exactMarker
  ) fail("exact current stable intent marker is required");
  if (process.env.CI === "true" || process.env.GITHUB_ACTIONS === "true") {
    fail("live Testnet write is disabled in CI");
  }
  ...
  const pool = new Pool({
    ...
    max: 4,
    application_name: "ipo-one-hypercore-002d-stable-once"
  });
  try {
    ...
    const intent = await repository.findByHash(intentHash);
    if (
    ...
    ) fail("stable exact intent is missing, consumed, stale or drifted");
    ...
    if (approvalDeadline <= approvedAt.getTime() + 1_000) {
      fail("stable intent approval window expired; generate a new exact marker");
    }
    const approval = createHypercoreStableFounderApproval({
      intent,
      actorId: FOUNDER_ACTOR_ID,
      confirmationNonceHash: hashId("hypercore_002d_stable_confirmation_nonce", {
        intentHash,
        exactMarker,
        nonce: randomUUID(),
    ...
    const output = await withHypercoreIsolatedTestnetSigner(
      ...
      async ({ descriptor, transientApiWalletAddress, signer }) => {
        if (
          descriptor.apiWalletAddressHash !== intent.apiWalletAddressHash ||
          descriptor.signerReferenceHash !== intent.signerReferenceHash
        ) fail("isolated signer identity drifted after approval");
        ...
        const composed = composeHypercore002dJitExecution({
          approvedIntent,
          approval,
          readiness,
          now: observedAt
        ...
        return {
          ...
          jitPostOnlyDistanceBps: composed.receipt.postOnlyDistanceBps,
          ...
          schemaVersion: "hypercore_002d_stable_once_result.v2"
        };
      }
    );
    console.log(`HYPERCORE_002D_STABLE_ONCE ${JSON.stringify(output)}`);
  } finally {
    await pool.end();
  }
}
