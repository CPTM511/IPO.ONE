...
const oldKeyPath=`${state}/web027k2-risk-authenticator.json`,newKeyPath=`${state}/web027m-review-authenticator.json`;
let activeGroup="m";
...
async function useRiskAuthenticator(){
 ...
 activeGroup="risk";
 for(const credential of JSON.parse(await readFile(oldKeyPath)).credentials)await cdp.send("WebAuthn.addCredential",{authenticatorId,credential});
}
...
try{
 for(const [port,name] of [[8939,"operations"],[8940,"auditor"],[8941,"risk-reviewer"]]){
  ...
  else{assert.equal(await page.locator("#loadLocalServicingBtn").isEnabled(),false);assert.equal(await page.locator("#localServicingForm").isVisible(),false);await opClick("#loadPilotCaseQueueBtn","pilotReadCaseQueue");assert.equal(await page.locator("[data-pilot-case-transition]:enabled").count(),0);}
  if(port===8939)await negative("pilotDecideApproval",{resource:{resourceType:"approval_proposal",resourceId:"approval_proposal_unavailable_acceptance"},payload:{expectedVersion:1,decision:"approve"},reasonCode:"approval_confirmed",idempotencyKey:`negative_decide_${crypto.randomUUID()}`});
  else await negative("pilotProposeApproval",{payload:{expiresAt:new Date(Date.now()+600000).toISOString(),command:{operationId:"pilotRestructureSandboxObligation",resource:{resourceType:"obligation",resourceId:"obligation_unavailable_acceptance"},payload:{expectedServicingStateHash:"0x"+"a".repeat(64),additionalTermDays:30},reasonCode:"sandbox_hardship_restructure",idempotencyKey:`negative_execute_${crypto.randomUUID()}`}},idempotencyKey:`negative_propose_${crypto.randomUUID()}`});
 ...
 await until(async()=>await page.locator("#createLocalSandboxAgentBtn").isVisible()||await page.locator("#createAccountChallengeBtn").isVisible(),"Principal Agent setup");
 if(await page.locator("#createLocalSandboxAgentBtn").isVisible()){
  const pending=page.waitForResponse(r=>r.url().endsWith("/local/v1/reference-agent/enrollment/create"));await click("#createLocalSandboxAgentBtn");const r=await pending;assert.equal(r.status(),200);const created=await r.json();assert.ok(created.subjectId);await writeFile(`${out}/freeze-target.json`,JSON.stringify({subjectId:created.subjectId}));
 }
 await page.locator("#createAccountChallengeBtn").waitFor({state:"visible"});await shot("disposable-agent-created");
 await useRiskAuthenticator();await go(8937);await login();await mfa();
 const directory=await opClick("#loadLocalAgentsBtn","pilotReadRiskAgentDirectory");assert.ok(directory.agents.length);const target=JSON.parse(await readFile(`${out}/freeze-target.json`));const agent=directory.agents.find(a=>a.subjectId===target.subjectId);if(agent){
 ...
 }else{results.push({name:"Previously frozen exact Agent remains excluded from the live directory",pass:true,subjectId:target.subjectId});}
 await go(8936);await loggedIn();await click(page.getByRole("button",{name:"Agents",exact:true}));await textIs("#agentAccountActivationStatus","Subject suspended");assert.equal(await page.locator("#createAccountChallengeBtn").isEnabled(),false);assert.equal(await page.locator("#proveAccountOnlineBtn").isEnabled(),false);assert.equal(await page.locator("#createDraftMandateBtn").isEnabled(),false);await shot("principal-after-freeze");results.push({name:"Principal recovers exact suspension and cannot start account proof or Mandate creation",pass:true});
 for(const port of [8939,8940,8941]){await go(port);await loggedIn();await textIs("#riskPasskeyBadge","Recently verified");}results.push({name:"Original Risk login also preserves all special-role sessions",pass:true});
