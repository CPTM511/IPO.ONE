...
try {
 ...
 assert.equal((await post('begin',{purpose:'register'})).status,400);results.push({name:'existing key prevents wallet-only re-enrollment',pass:true});
 for(const kind of ['signature','origin','rp','handle','crossOrigin','rawId','unknownField']){
  const body=await prepareAssertion();
  const encode=x=>Buffer.from(x).toString('base64url');
  if(kind==='signature')body.response.response.signature=encode(Buffer.alloc(70,1));
  if(kind==='rp'){const bytes=Buffer.from(body.response.response.authenticatorData,'base64url');bytes[0]^=1;body.response.response.authenticatorData=encode(bytes);}
  if(kind==='handle')body.response.response.userHandle=encode(Buffer.alloc(32,1));
  if(kind==='rawId')body.response.rawId=encode(Buffer.alloc(32,1));
  if(kind==='unknownField')body.response.grantRisk=true;
  if(['origin','crossOrigin'].includes(kind)){const data=JSON.parse(Buffer.from(body.response.response.clientDataJSON,'base64url'));if(kind==='origin')data.origin='https://attacker.invalid';else data.crossOrigin=true;body.response.response.clientDataJSON=encode(JSON.stringify(data));}
  const result=await post('finish',body);assert.equal(result.status,400);assert.equal((await status()).verified,false);
  assert.equal((await post('finish',body)).status,400);results.push({name:`reject ${kind} and replay without granting MFA`,pass:true});
 }
 const cancelled=await post('begin',{purpose:'verify'});assert.equal(cancelled.status,200);assert.equal((await post('cancel',{challengeId:cancelled.body.challengeId})).status,200);
 assert.equal((await post('cancel',{challengeId:cancelled.body.challengeId})).status,400);assert.equal((await status()).verified,false);results.push({name:'cancel consumes challenge without verification',pass:true});
 const valid=await prepareAssertion();const pair=await Promise.all([post('finish',valid),post('finish',valid)]);assert.deepEqual(pair.map(r=>r.status).sort(),[200,400]);results.push({name:'concurrent same challenge has exactly one accepted verification',pass:true});await saveAuthenticator();
 await w.page.reload();await expect(w.page.locator('#riskPasskeyBadge')).toHaveText('Recently verified',{timeout:20000});
 await expect(w.page.locator('#riskPortfolioHelper')).toContainText('Authorized point-in-time exposure loaded',{timeout:20000});
 await w.page.locator('#loadRiskInsightsBtn').click();await expect(w.page.locator('#loadRiskInsightsBtn')).toBeEnabled({timeout:20000});
 await w.page.locator('#loadPilotCaseQueueBtn').click();await expect(w.page.locator('#loadPilotCaseQueueBtn')).toBeEnabled({timeout:20000});
 await w.page.locator('#loadClosedPilotReadinessBtn').click();await expect(w.page.locator('#loadClosedPilotReadinessBtn')).toBeEnabled({timeout:20000});
 await w.page.screenshot({path:out+'/verified-insights.png',fullPage:true});await writeFile(out+'/verified-insights.txt',await w.page.locator('body').innerText());
 results.push({name:'visible portfolio, health, feedback, case queue and readiness controls',pass:true});
...
} catch(e){results.push({error:e.message,stack:e.stack?.split('\n').slice(0,4)});if(w){await w.page.screenshot({path:out+'/security-failure.png',fullPage:true});await writeFile(out+'/security-failure.txt',await w.page.locator('body').innerText());}process.exitCode=1;}
finally{await saveAuthenticator();await writeFile(out+'/security.json',JSON.stringify({source:JSON.parse(await readFile('output/playwright/web-027/candidate-runtime.json')).source,apiMocks:false,virtualAuthenticator:true,founderDeviceVerified:false,results,operationResults,errors},null,2));await browser.close();console.log(JSON.stringify(results));}
