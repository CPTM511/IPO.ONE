...
try{
 ...
 for(const phase of ["login","refresh","relogin"]){
  ...
  await expect(page.locator("#humanObligationStatus")).toBeVisible();await expect(page.locator("#humanObligationStatus")).toHaveText(/Fully repaid/i,{timeout:30000});await expect(page.locator("#humanObligationOutstanding")).toHaveText("$0.00");await expect(page.locator("#humanObligationRepaid")).toHaveText("$24.50");
