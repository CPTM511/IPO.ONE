...
const queries={
 activationEvents:"SELECT event_type, payload->>'previousStatus' AS previous, payload->>'nextStatus' AS next, count(*)::int AS count FROM domain_events WHERE payload->>'reasonCode'='explicit_local_synthetic_activation' GROUP BY event_type,payload->>'previousStatus',payload->>'nextStatus'",
 duplicateActivations:"SELECT count(*)::int AS count FROM (SELECT subject_id FROM domain_events WHERE payload->>'reasonCode'='explicit_local_synthetic_activation' GROUP BY subject_id HAVING count(*)>1) duplicate",
 previousGeneration:"SELECT status,count(*)::int AS count FROM authentication_credentials WHERE client_id IN ('client_web027j_actor_human_borrower_pilot','client_web027j_actor_principal_controller_pilot') GROUP BY status",
 activationCapability:"SELECT roles, count(*)::int AS count FROM authentication_credentials WHERE status='active' AND allowed_capabilities ? 'subject.activate.sandbox.self' GROUP BY roles",
 migrations:"SELECT count(*)::int AS count,max(name) AS latest FROM schema_migrations",
 ...
 newRotation:"SELECT status, count(*)::int AS count FROM authentication_credentials WHERE client_id IN ('client_web027k_actor_human_borrower_pilot','client_web027k_actor_principal_controller_pilot') GROUP BY status"
};
const results={};
...
const previous=docker(['logs','ipo-one-web027-candidate-dc23daf46313']);
...
await writeFile(out+'/k1/credential-rotation-manifest.json',JSON.stringify({entries:manifest},null,2));
...
await writeFile(out+'/k1/local-access-audit.json',JSON.stringify(audit,null,2));console.log(JSON.stringify(audit));
