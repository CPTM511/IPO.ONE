...
<title>IPO.ONE Agent Credit Execution Boundary</title>
<style>
...
<p class="eyebrow">AGENT-CREDIT-EXEC-001 · Founder Review</p><h1>Agent Credit Execution Boundary</h1>
<p class="lead">外部 Agent 只通过可替换的 CreditProvider / ExecutionVenue 接口，完成授信、受控执行、对账、优先还款与证据回写。这里运行的是确定性 L0 模拟，不连接 Testnet，不移动资金。</p>
...
<div class="grid">
<section class="card"><h2>正常闭环</h2><p>最终权益 1,100；本金 1,000 先回款，剩余 100 后释放。</p><button data-scenario="healthy">运行正常流程</button><pre id="healthy">等待运行…</pre></section>
<section class="card loss"><h2>亏损闭环</h2><p>最终权益 960；部分回款后保留 40 未偿，不伪装为已结清。</p><button data-scenario="loss">运行亏损流程</button><pre id="loss">等待运行…</pre></section>
</div>
<div class="path"><strong>实际路径</strong><br>External Agent → authenticate → CreditIntent → Capital Partner Offer → Facility → controlled account → open/close → reconcile → canonical repayment → Evidence / CreditState</div>
</main><script>
const labels={stage:'生命周期',facilityId:'Facility',executionCount:'受控执行数',reconciliationStatus:'对账',appliedPrincipalMinor:'已还本金',outstandingPrincipalMinor:'未偿本金',residualReleaseMinor:'后释放余额',creditState:'CreditState',riskState:'RiskState',evidenceCount:'证据数',agentCustody:'Agent 托管',withdrawalAuthority:'提款权限',transferAuthority:'转账权限',realFundsMoved:'真实资金',mainnetInteraction:'主网交互',testnetAssetUsed:'Testnet 资产'};
