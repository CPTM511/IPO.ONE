...
const config=JSON.parse(docker(["inspect","ipo-one-web027-candidate"]))[0];
const url=new URL(config.Config.Env.find(v=>v.startsWith("DATABASE_URL=")).slice(13));
assert.equal(url.hostname,"127.0.0.2");assert.equal(url.port,"55435");assert.equal(url.username,"ipo_one_owner");url.pathname="/"+database;
...
const copy=spawnSync("limactl",["shell","--workdir","/Users/cptmao/Documents/IPO.ONE","ipo-one-local","docker","cp","-",container+":/app"],{input:archive.stdout,maxBuffer:16*1024*1024});assert.equal(copy.status,0,copy.stderr?.toString());
