...
const health=[];for(const origin of ['http://127.0.0.1:8935','http://127.0.0.1:8936','http://localhost:8937','http://127.0.0.1:8938',...(runtime.additionalOrigins??[])]){const response=await fetch(origin+'/tenant/v1/healthz');assert.equal(response.status,200);health.push({origin,status:response.status});}
