...
| Agent API contract | Deployed and authenticated end-to-end in the closed no-funds sandbox at `f8bc87c...` | Closed 103-operation private catalog, JSON Schemas, fixtures and stable errors; the owner-controlled Golden Flow Agent completed A-01 through A-09 over the production SDK/MCP transport with DPoP, exact Mandate/Offer/Obligation binding, idempotent repayment, 21 finalized Evidence items, terminal Credit Outcome, durable Credit State and separate-process recovery; the temporary Credential was then revoked and its private key destroyed | Public credential issuance, current-user chain writes, real funds and permanent Golden fixture retention remain unavailable and require separate approval |
...
| Authenticated HTTP | Founder wallet verified as one canonical Human with explicit `principal_controller` and `human_borrower` enrollments; one selected role per SIWE/session; full Human no-funds lifecycle and post-deploy recovery verified at `f8bc87c...` | Migration 0063, forced RLS, durable PII-free role-enrollment/selection events, one-use SIWE role binding and request-time enrollment revalidation preserve Tenant, Subject, ownership, Consent and least privilege; H-03 through H-14 completed through visible controls, including two repayments, terminal outcome, Track Record, Decision Passport, logout/login and refresh recovery | Backup/restore exercise, independent review, production identity/KYC and all real-value authority remain separate gates |
| Agent MCP | Deployed authenticated A-01 through A-09 acceptance complete in the closed no-funds sandbox | A deterministic external workload client used a new P-256 DPoP key, public-only bootstrap, production SDK/MCP contract and durable services; it completed the shared lifecycle, idempotent repayment, owned Evidence/Passport/Credit State and restart recovery; post-acceptance revocation failed closed and private material was destroyed | No permanent test Agent is retained; public credential issuance, current-user chain writes, withdrawals and real funds remain unavailable |
