...
local SDK and the exact thirteen-tool authenticated local-stdio MCP registry,
...
| Agent API contract | Verified local through Decision Passport, exact current Obligation read, owned Evidence, outcome-derived Credit State, repayment and portability; exact contract and fail-closed authentication are deployed at `a1319d30...` | Closed 103-operation private catalog, JSON Schemas, fixtures and stable errors; UI separates retained public test fixtures, private Tenant operations and thirteen local MCP tools; `pilotReadServicingQueue` is explicitly excluded from Agent MCP, while `pilotReadOwnObligation` / `ipo_one_read_obligation`, `pilotPostSandboxRepayment`, `pilotReadOwnObligationEvidence`, and `pilotReadOwnCreditState` retain the owner-bound shared lifecycle path; `pilotReadCreditRegistryEvidence` remains a separate Tenant-authorized, read-only, synthetic-only chain proof and is not an owned repayment event; Credit State is qualitative, outcome-derived, and non-authorizing; Human session state remains hidden in Agent mode; the three Provider operations remain absent from Agent MCP; handoff/capability manifests and lifecycle SDK clients retain closed replay-safe receipts | The production HTTPS contract is hosted, but authenticated Agent E2E remains blocked because the reviewed external workload private JWK is unavailable; public credential issuance, current-user chain writes and real funds remain unavailable |
...
| Authenticated HTTP | Founder Principal Controller SIWE, durable workspace recovery, visible logout and post-deploy persistence verified on hosted SHA `a1319d30...` | The hosted `apps/tenant-api` composition wires Human session/Agent workload resolver, trusted Network Context, CSRF bootstrap and fixed UI module graph; AUTHN-004 adds forced-RLS PostgreSQL Credentials, encrypted one-use OIDC/SIWE transactions, restart-safe sessions, immutable events, atomic Credential/session deprovisioning, an authentication-only database role, and `createPostgresHumanAccessComposition` startup verification; `createHumanAccessRouteHandler` composes truthful options, provider-bound OIDC initiation/callback, same-origin SIWE challenge/verify, and CSRF-protected logout with Secure host-only cookies | Full Human lifecycle acceptance remains blocked because no invited Human Borrower credential is present; final post-redeploy wallet approval, backup/restore exercise and independent review remain open |
| Agent MCP | Verified locally through exact owned Obligation/Evidence reads, durable Credit State, clearly historical public synthetic Registry Evidence and the no-funds lifecycle; hosted API advertises the equivalent versioned operation contract | The fixed thirteen-tool registry provides application/Offer, CAIP-10 proof/binding, exact owned Obligation, owned Evidence, durable Credit State/Track Record, separate historical Registry Evidence, Offer acceptance, sandbox execution and synthetic repayment; `createAgentPilotHost` composes fresh Host-owned authentication, trusted Network Context, exact handoff Subject binding and actual local stdio | Remote authenticated execution is not verified because the external DPoP private JWK is unavailable; no key may be copied into Vercel or the browser; current-user chain writes, withdrawals and real funds remain unavailable |
