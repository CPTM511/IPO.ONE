...
| Authenticated HTTP | Verified local adapter and embedding composition | Separate loopback-only `apps/tenant-api`; closed `createTenantPilotHost` wires Human session/Agent workload resolver, trusted Network Context, CSRF bootstrap and fixed UI module graph; three closed routes and conformance tests | Approved HTTPS OIDC login/callback and production dependencies remain uncomposed; public/remote remains disabled |
