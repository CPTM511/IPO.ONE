...
| Agent MCP | Verified locally through exact owned Obligation/Evidence reads, durable Credit State, clearly historical public synthetic Registry Evidence and the no-funds lifecycle | The fixed thirteen-tool registry provides application/Offer, CAIP-10 proof/binding, exact owned Obligation, owned Evidence, durable Credit State/Track Record, separate historical Registry Evidence, Offer acceptance, sandbox execution and synthetic repayment; `createAgentPilotHost` composes fresh Host-owned authentication, trusted Network Context, exact handoff Subject binding and actual local stdio | Production credential/deployment, remote/public MCP, current-user chain writes, withdrawals and real funds remain unavailable |
