...
| FR-011 Event Indexer | Rail event replay, multi-event PostgreSQL runtime, materialized core projections and reconciliation; provider-neutral synthetic chain indexer plus bounded live-testnet observers, durable Tenant-RLS stores, hash-only outbox and replay reconciliation | CHAIN-001A/001C local portability and CHAIN-001B dual-chain read-only access are verified. CHAIN-001D completes one synthetic Base Sepolia Credit Registry lifecycle; CHAIN-001E independently verifies both RPC views and persists its closed, paused, inactive result through a dedicated immutable Tenant-RLS observation/outbox boundary. No production finality or mainnet approval exists | Complete remaining CHAIN-001B signed emitter receipts; INDEXER-001 production design |
...
`CHAIN-001D` subsequently completed one separately approved synthetic Base
Sepolia Credit Authorization Registry lifecycle. `CHAIN-001E` now observes its
publication, proof-update, close and pause transactions through both fixed RPC
slots, verifies zero value, calldata/event binding, stable blocks and final
closed state, and persists a hash-only non-authorizing aggregate through
dedicated Tenant-RLS tables. The live evidence uses no signer and broadcasts no
transaction. Neither issue authorizes mainnet, production credit or real funds.
