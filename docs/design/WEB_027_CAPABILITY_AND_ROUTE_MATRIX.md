...
## WEB-027M delta — installed `205fe9b`, 2026-09-07

| Role / capability | Local CODE / RUNTIME / installation | Visible reachability and verification | Remaining gate |
| --- | --- | --- | --- |
| Operations · localhost:8939 | Installed; exact approved grant | Signed login, native Passkey, approval inbox, servicing queue, refresh/re-login/restart; cannot approve own/other proposals without a granted approval capability | Independent Operations reviewer; legitimately eligible plan |
| Auditor · localhost:8940 | Installed; read-only exact grant | Signed login, native Passkey, portfolio/case/inbox reads; mutation controls disabled and server denies wrong-role writes; cookie isolation verified | Actual populated servicing proposal review |
| Independent Risk Reviewer · localhost:8941 | Installed; exact approval-only grant | Signed login, native Passkey, inbox/queue reads; cannot propose or execute; independent session recovery | Real eligible proposal and second independent reviewer |
| Risk protective Agent selector | Installed | Visible exact selection, reason, acknowledgement, freeze; one actual pending → suspended event; refresh and final Principal recovery | Physical device matrix only; no unfreeze enabled |
| Three future servicing plans | Real deployed local services and persistence | Three separate visible Human acceptance/execution flows, each $1; normal due time September 8 about 17:34–17:36 Asia/Shanghai | Not yet delinquent; write-off requires defaulted/restructured/repurchased state |

Final web 205 tests and PostgreSQL 99 tests pass with appropriate source attribution; actual Human/Agent/Capital regressions run at installed `205fe9b`. See `web-027/m-verification-evidence.json`. Earlier missing-role and queue-only-freeze statements above are historical checkpoints, superseded only for the precise M rows here. Formal website remains NOT DEPLOYED for every M row. Whole-site verdict remains BLOCKED — NOT COMPLETE.
