...
The approved sidebar presentation update is **PASS — DEPLOYED AND USER-VERIFIED** through delegated browser acceptance. Whole-site delivery remains **BLOCKED — NOT COMPLETE** with the recorded original Principal owned-Agent visible acceptance; no 100% claim is made. Current local API/worker and formal runtime source is `55c6019475c1e2990fee1199674545e0f4526b79`.

Current experience: https://ipo.one. Deployment `dpl_EZptaVTiDpNn1LN85sA4zKppEpiW` retains the original environment and 76 hosted migrations. The sidebar shows a compact Beta label and natural-language funds statement; environment details open on demand and API reference remains available under More tools for every role. Both required CI gates passed, along with 205 web tests and focused four-role/keyboard/mobile checks. Real local and formal clicks opened the API specification and verified details in Light/Dark. Formal login, refresh and logout/login recovered the original fully-repaid $24.50 record and ten Evidence events; startup frame checks found no old-UI flash. All 90 local runtime file hashes and eight role endpoints passed.

Original OKX login recovery remains confirmed by the native authenticated Principal page and canonical audit. Final-source owned-Agent navigation remains separately unverified; earlier Agent and servicing evidence retains its source. Rollback restores `dpl_ESCjVFTwCyBFJxWvZuwuB2F8Layn` with no database rollback. Details: [existing release task](../codex/tasks/WEB_027I_RELEASE.md) and [source-bound evidence](web-027/formal-release-evidence.json). Earlier dated sections are historical checkpoints.
