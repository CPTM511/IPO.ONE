...
The workspace Dark/Light readability repair is **PASS — DEPLOYED AND USER-VERIFIED** through delegated browser acceptance. Whole-site delivery remains **BLOCKED — NOT COMPLETE** for the separately recorded original Principal owned-Agent visible acceptance. Current local API/worker and formal runtime source is `9b388b2702ae82b21e65798d9e9a6dbe6dc9f613`.

Current experience: https://ipo.one. Deployment `dpl_9Thjh3YJCzr9YR29fTZJc3qe6gqh` replaces legacy light-only card surfaces and fixed text colors with the active workspace palette, including activity rows, recovery banners, receipts, technical details and workspace action links. Actual hosted Human/Principal journeys pass both themes at desktop/mobile widths through visible login, navigation, disclosure, hover/focus and refresh. Separate four-role component regression covers Capital/Risk styling. Formal wallet login/refresh/relogin preserves the fully-repaid $24.50 record and ten owner Evidence events. Both exact-source CI runs pass; hosted asset hashes and 90 local runtime files/eight role endpoints match.

No API, permission, dependency, schema, economic action or production configuration change. The existing 76-migration hosted profile remains. Rollback: `dpl_H6bnc1ddaBsNdNd9GCggV2UHS6xg` (`3246672`). Previous startup, footer, wallet and lifecycle evidence retain their actual sources; the eight public startup paint cases were not repeated for this CSS repair. No whole-site 100% claim or completed-acceptance merge. Details: [release task](../codex/tasks/WEB_027I_RELEASE.md) and [source-bound evidence](web-027/formal-release-evidence.json).
