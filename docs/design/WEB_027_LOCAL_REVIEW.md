...
The September 10 startup-flash repair is **PASS — DEPLOYED AND USER-VERIFIED** through delegated browser acceptance. Whole-site delivery remains **BLOCKED — NOT COMPLETE** with the recorded original Principal owned-Agent visible acceptance; no 100% claim is made. Current local API/worker and formal runtime source is `5ede04b45877cdfe483940e18dfc9bcaf907e30b`.

Current experience: https://ipo.one. Deployment `dpl_ESCjVFTwCyBFJxWvZuwuB2F8Layn` retains the original environment and 76 hosted migrations. A theme-correct startup surface replaces the old-UI flash, authenticated workspace recovery completes before reveal, and the 37-module browser entry ships as one compiled asset. Both required CI gates passed. Actual formal visible login, refresh, logout/login and animation-frame observations found no legacy frame or public-page flash during authenticated recovery; the original $24.50 fully-repaid record and ten Evidence events recovered. All 90 local runtime file hashes match and eight role endpoints are healthy; compiled frontend Human/Capital recovery passed against durable local services.

Original OKX login recovery is now confirmed by the authenticated native Principal page and read-only canonical audit retaining the same identity. Original owned-Agent navigation on the final SHA remains unverified because of concurrent user browser interaction. Earlier Agent machine lifecycle and local servicing evidence retain their original sources. Rollback restores the prior compatible 76-migration deployment `dpl_6C4nXHCG4P5QUhVdJ755xd88ZShK` with no database rollback. Details: [existing release task](../codex/tasks/WEB_027I_RELEASE.md) and [source-bound evidence](web-027/formal-release-evidence.json). Earlier dated sections are historical checkpoints.
