...
The residual startup flash repair is **PASS — DEPLOYED AND USER-VERIFIED** through delegated browser acceptance. Whole-site delivery remains **BLOCKED — NOT COMPLETE** for the separately recorded original Principal owned-Agent visible acceptance. Current local API/worker and formal runtime source is `3246672c38dd94d9b29d9d4a98a8d65764fffe11`.

Current experience: https://ipo.one. Deployment `dpl_H6bnc1ddaBsNdNd9GCggV2UHS6xg` removes the full-screen loading interstitial; slower loads receive a quiet delayed status, with failed-load retry and disabled-JavaScript guidance preserved. The exact hosted asset hashes match the bundle. Eight formal desktop/mobile, Light/Dark cold/reload paint cases and visible login entry pass, as do real wallet login/refresh/logout-login with the existing fully-repaid record and owner Evidence. Final-source CI passes; local runtime checks match all 90 files and all eight role endpoints. The existing environment and 76 hosted migrations remain unchanged. Rollback is the compatible `55c6019` deployment `dpl_EZptaVTiDpNn1LN85sA4zKppEpiW`. Previous footer, wallet and historical lifecycle evidence remain source-bound in the manifest. No whole-site 100% claim or completed-acceptance merge.

Original OKX login recovery remains independently confirmed; final-source owned-Agent acceptance remains the separate parent item. Details: [release task](../codex/tasks/WEB_027I_RELEASE.md) and [source-bound evidence](web-027/formal-release-evidence.json). Earlier dated sections retain their historical sources.
