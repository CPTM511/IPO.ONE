...
Formal checkpoint: https://ipo.one now serves `7e83916`; local API/worker use that same source. Required CI passed, the signed-wallet login defect was reproduced on the old formal release and fixed on the new one, and the full Human lifecycle plus visible refresh/re-login recovery passed with $24.50 repaid and nine owner Evidence events. The release is not declared fully accepted: the formal Principal test identity has no assigned Agent, and a current authorized connection is required for the remaining hosted Agent flow. See `web-027/formal-release-evidence.json`; do not substitute local Agent receipts or a Ready deployment for that result.
