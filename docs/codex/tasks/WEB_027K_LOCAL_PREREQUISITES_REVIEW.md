...
K1 已实现并部署到隔离候选，当前运行源码 `5303c90`，迁移 0079；新旧 Human 可见激活、账户绑定以及撤销/刷新/注销重登/重启证据见 `docs/design/WEB_027_LOCAL_REVIEW.md`。原服务保留，未正式发布。

K2 的实现授权继续有效；没有伪造 MFA 或放宽原策略。新增验证库精确为 `@simplewebauthn/server@14.0.1`，按本提案原有条款提交了独立具体审阅，等待回复。浏览器已实测 127.0.0.1 RP 不可用，localhost 的精确来源方案与 durable session evidence/撤销设计在 ADR-WEB-027K2 中。整体仍 BLOCKED — NOT COMPLETE。
