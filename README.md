...
| Account access | OIDC Authorization Code + PKCE BFF for Google/email/common IdPs; one-use SIWE wallet sessions; active internal Credential mapping; Secure host-only sessions and CSRF; provider-bound same-origin login/callback/challenge/verify/logout HTTP composition. Public activation remains deployment-gated |
...
| Authentication | Human OIDC/PKCE or SIWE host sessions and sender-bound Agent/Provider/system identity | Approved local non-funds foundation with standard-provider subject mapping, provider-bound OIDC HTTP initiation/callback, same-origin one-use SIWE challenge/verify, logout, active internal Actor/Credential binding, DPoP/mTLS, session/CSRF controls, and credential-free lifecycle events; public activation remains deployment-gated |
