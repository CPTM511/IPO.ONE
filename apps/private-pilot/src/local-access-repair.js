...
export const LOCAL_ACCESS_GENERATION = "web027j";
...
export async function rotateLocalAccessCredentials({ pool, tenantId, identities, basePort, now = new Date() }) {
  ...
  try {
    ...
    await client.query("SELECT pg_advisory_xact_lock(hashtext('web027j_local_credential_generation'), hashtext($1))", [tenantId]);
    for (const [index, name] of ["borrower", "controller"].entries()) {
      ...
      const rows = await client.query(`SELECT c.*, m.capabilities AS membership_capabilities, m.client_ids AS membership_client_ids FROM authentication_credentials c
        ...
        WHERE c.tenant_id=$1 AND c.issuer=$2 AND c.client_id=$3
        ...
        ORDER BY c.id FOR UPDATE OF c,m`, [tenantId,issuer,oldClientId,now]);
      for (const c of rows.rows) {
        ...
        if (!enrollments.rowCount || enrollments.rows.some(e => e.status !== "active" ||
            ...
            !e.client_ids.includes(oldClientId))) continue;
        ...
        await client.query(`UPDATE memberships SET capabilities=$3::jsonb, client_ids=$4::jsonb,
          ...
          [tenantId,c.actor_id,JSON.stringify(localAccessCapabilities(c.membership_capabilities)),JSON.stringify([...new Set(c.membership_client_ids.map(value => value === oldClientId ? newClientId : value))]),now]);
        ...
        for (const [eventType, credentialId, payload] of [
          ...
          VALUES($1,$2,$3,'actor_local_authentication_system',$4,'web027j_local_access_rotation',$5,$6::jsonb,'authentication_event.v1')`,
        ...
        manifest.push({ generation:LOCAL_ACCESS_GENERATION,actorId:c.actor_id,oldCredentialId:c.id,newCredentialId:id,
          oldClientId,newClientId,before:c.allowed_capabilities,after:capabilities,previousSessionsRevoked:true });
      }
    }
