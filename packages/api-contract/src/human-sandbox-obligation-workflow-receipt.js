...
export function isHumanSandboxObligationWorkflowReceipt(value) {
  return validateReceipt(value) === true;
}

export function assertHumanSandboxObligationWorkflowReceipt(value) {
  if (!isHumanSandboxObligationWorkflowReceipt(value)) {
    throw new DomainError(
      "invalid_human_sandbox_obligation_workflow_receipt",
      "Human sandbox Obligation workflow receipt does not satisfy its versioned contract"
    );
  }
}
